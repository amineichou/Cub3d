...
/*   Updated: 2024/07/09 13:01:52 by moichou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_extension(char *ext, int i)
{
	...
	if (ext[i] == '\0' || ft_strcmp(&ext[i], "cub") != 0)
		return (ft_printerror("invalid extension for map filename (.cub)\n"), -1);
	return (0);
}
...
static int	check_filename(char *filename)
{
	...
	if (no_extension)
		return (ft_printerror("invalid file, file has no extension (.cub)\n"), -1);
	return (1);
}
